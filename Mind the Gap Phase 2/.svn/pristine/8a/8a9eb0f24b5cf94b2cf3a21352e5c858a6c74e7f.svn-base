...
public class ArrivalsListFragment extends ListFragment {
    ...
    public ArrayList<Arrival> getArrivalsForSelectedStationOnLineInDirection(String lineId, String travelDirn) {
        ...
        Iterator<ArrivalBoard> itr = selectedStation.iterator();

        while (itr.hasNext()) {
            ArrivalBoard board = itr.next();

            if (board.getLine().getId().equals(lineId) &&
                    board.getTravelDirn().equals(travelDirn)) {

                Iterator<Arrival> arrivalItr = board.iterator();
                while (arrivalItr.hasNext()) {
                    Arrival arrival = arrivalItr.next();
